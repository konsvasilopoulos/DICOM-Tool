...
tab1, tab2, tab3 = st.tabs(["🔒 DICOM Anonymizer", "🔍 Inspector & 3D Viewer", "📊 Batch CSV Report Generator"])
...
with tab2:
    ...
    st.markdown("Upload a **single DICOM file** (`.dcm`) OR a **ZIP archive** containing a 3D CT/PET series to navigate slices.")
    ...
    if uploaded_input is not None:
        file_extension = uploaded_input.name.split('.')[-1].lower()
        try:
            if file_extension == "zip":
                with zipfile.ZipFile(uploaded_input, 'r') as z_in:
                    ...
                    # Sort datasets by Image Position Patient (Z-axis) if available
                    def get_z_pos(d):
                        try:
                    ...
                    temp_datasets.sort(key=get_z_pos)
    ...
    if len(datasets_list) > 0:
        # Select active dataset using slice slider if multiple slices exist
        total_slices = len(datasets_list)
        if total_slices > 1:
            st.info(f"📌 **3D Volume Detected:** `{total_slices} slices` found in dataset.")
            slice_index = st.slider("🛞 3D Slice Navigator (Z-Axis)", min_value=1, max_value=total_slices, value=1, step=1)
            ds = datasets_list[slice_index - 1]
            st.markdown(f"**Active Slice:** `{slice_index} / {total_slices}`")
        else:
            ds = datasets_list[0]
            slice_index = 1
            total_slices = 1
        ...
        if modality == "CT":
            ...
            st.info(f"📌 Detected Modality: **PET (Positron Emission Tomography)** — Functional imaging active.")
        elif modality == "MG":
        ...
        with col_left_viewer:
            ...
            if hasattr(ds, "pixel_array"):
                ...
                img_buf.seek(0)
                st.download_button("📥 Download Preview PNG", img_buf, file_name=f"dicom_slice_{slice_index}_preview.png", mime="image/png")
        ...
        with col_right_controls:
            ...
            with st.expander("✏️ DICOM Header Editor & Fixer", expanded=False):
                ...
                if st.button("💾 Apply Edits & Download Active DICOM"):
        ...
        if st.session_state.get("enable_line_profile", False):
            ...
            if num_points > 1:
                ...
                ax_esf.set_title(f"Edge Spread Function (ESF) — Slice {slice_index}", fontsize=11, weight='bold')
                ...
                if np.sum(lsf) > 0:
                    ...
                    if fft_vals[0] > 0:
                        ...
                        try:
                            mtf_50_freq = freqs[np.argmin(np.abs(mtf - 0.5))]
                        except:
                            mtf_50_freq = 0.0
                            
                        try:
                            mtf_10_freq = freqs[np.argmin(np.abs(mtf - 0.1))]
                        except:
                            mtf_10_freq = 0.0
        ...
        with col_meta1:
            with st.expander("📋 Active Slice Metadata Summary"):
                info = {
                    "Modality": modality,
